/**
 * DatLichPage.jsx
 * Đặt lịch hẹn — kết nối đầy đủ với backend
 *
 * THAY ĐỔI SO VỚI PHIÊN BẢN CŨ:
 * [MỚI] Dịch vụ lấy từ API thật (GET /api/dichvu/search-all?status=Hoạt động)
 * [MỚI] Bác sĩ lấy từ API thật (GET /api/users/bac-si/hoat-dong)
 * [MỚI] Khách hàng tìm kiếm debounce qua API
 * [MỚI] Parse mã lịch hẹn từ response backend
 * [MỚI] Format ngayHen/gioHen đúng kiểu Java LocalDate/LocalTime
 * [SỬA] Admin/NhanVien → trạng thái "Đã xác nhận" luôn
 * [SỬA] BenhNhan → trạng thái "Chờ xác nhận"
 */
...
export default function DatLichPage({ S, role = "benhnhan", onBack }) {
  ...
  // ── Step DV & BS ──────────────────────────────────────────
  const [dvList, setDvList] = useState([]); // [{ maDichVu, tenDichVu, donGia }]
  const [dvLoading, setDvLoading] = useState(true);
  const [selectedDV, setSelectedDV] = useState([]); // [maDichVu]
  const [dvSearch, setDvSearch] = useState("");
  const [bsList, setBsList] = useState([]); // [{ id, ten, sdt }]
  ...
  // [MỚI] Load dịch vụ hoạt động từ API thật
  useEffect(() => {
  ...
  useEffect(() => {
    // Chưa chọn bác sĩ: giữ danh sách giờ mặc định.
  ...
  // [MỚI] Submit đặt lịch — format ngayHen đúng ISO, gioHen "HH:mm"
  const handleSubmit = async () => {
    ...
    try {
      ...
      const body = {
        ...
        ngayHen, // "YYYY-MM-DD" → backend LocalDate
        gioHen, // "HH:mm"      → backend LocalTime
        ...(hasKHStep ? { maKhachHang: selectedKH.id } : {}),
      };
      const res = await datLichHenAPI(body);
      // [MỚI] Parse mã từ response: "Đặt lịch hẹn thành công! Mã lịch hẹn: LH_0001"
      const match =
        ...
        <div style={S.card}>
          ...
          >
            <div>
              ...
              <div style={{ fontSize: 11, marginBottom: 6, visibility: "hidden" }}>
                .
              </div>
              <input
                type="date"
            ...
            <div>
              ...
              >
                {!gioTrongList.length && <option value="">Không còn ca trống</option>}
                {gioTrongList.map((g) => (
