...
export default function DangKyKhamPage({ S }) {
  ...
  const [searchQ, setSearchQ] = useState(""); // SĐT | tên | mã KH
  const [searching, setSearching] = useState(false);
  const [searchResults, setSearchResults] = useState([]); // kết quả tìm
  ...
  // ── Tìm kiếm theo SĐT, tên, hoặc mã KH ──────────────────
  const handleSearch = async () => {
    ...
    try {
      ...
      if (res.length === 0) {
        ...
        setFoundKH(res[0]); // tự động chọn nếu chỉ có 1 kết quả
      } else {
        setSearchResults(res); // hiển thị danh sách để chọn
      }
    } catch (e) {
      setError(e.message);
    } finally {
  ...
  // ── Tạo bệnh nhân mới + lấy mã KH từ response ────────────
  const handleCreateNew = async () => {
    ...
    try {
      ...
      // Parse mã KH từ response: "Đăng ký Khách hàng thành công! \nMã Khách Hàng: KH_0001\n..."
