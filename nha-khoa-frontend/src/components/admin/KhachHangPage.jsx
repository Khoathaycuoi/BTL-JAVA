...
import { getAllKhachHangAPI, getKhachHangHoatDongAPI } from "../../api/User";
...
function AddKhachHangModal({ onClose, onSuccess }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      setLoading(true);
      setSubmitErr("");
      await registerKhachHangAPI(form);
  ...
  return (
    <Modal title="➕ Thêm khách hàng mới" onClose={onClose} width={540}>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        ...
        <div style={{ gridColumn: "1/-1" }}>
          <FormField
            label="Tiền sử bệnh"
            type="textarea"
            placeholder="Dị ứng thuốc, bệnh nền... (không bắt buộc)"
...
const ROLE_LABEL = { KHACH_HANG: "Khách hàng" };

export default function KhachHangPage({ S, onlyActiveKhachHang = false }) {
  const { theme: C } = useTheme();
  ...
  const fetchKhachHang = useCallback(
    (keyword) =>
      onlyActiveKhachHang
        ? getKhachHangHoatDongAPI(keyword)
        : getAllKhachHangAPI(keyword),
    [onlyActiveKhachHang],
  );
  const showToast = (msg) => {
    setToast(msg);
